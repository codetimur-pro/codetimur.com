/* scenes-6-7.jsx — Проводник (particle silhouette) / Миссия (dawn pulse)
   Exports: Scene6, Scene7 */
(function () {
  const e = React.createElement;
  const { useRef, useEffect } = React;
  const U = window.CTlib;

  /* ---- S6MtnFX: mountain scales from Scene5 full → 1/3, trail stays ---- */
  function S6MtnFX({ active }) {
    ...
    const ref = window.useFxCanvas({
      ...
      frame: (ctx, w, h, t, st) => {
        ...
        // Scale 1.0 → final over 1.6s; mobile: larger final size + shift cx right
        ...
        // Mountain body (70% opacity — in foreground, semi-transparent)
  ...
  function Scene6({ active }) {
    return e('section', { className: 'scene s6', 'data-active': active, 'data-screen-label': '06' },
      ...
      e(window.StarField, { density: 0.30, twinkle: true, radiusScale: 0.92, yMax: 0.55 }),
      e(S6MtnFX, { active }),
  ...
  /* ===================== SCENE 7 — MISSION (Night Earth) ===================== */
  /* Holographic gold point-cloud Earth — raw Three.js particle globe.
     Sized 1/1.5 vs previous. Entry: scale 0→1 + radial bloom pulse on active.
     Land from earth-water (binary mask) + earth-night (city hotspots).
     AdditiveBlending glow, opacity 0.80, slow Y spin. */
  function EarthGlobe({ active }) {
    ...
    // Trigger appear animation whenever active flips to true
    useEffect(() => {
      if (!active) return;
      const delay = 320; // sync with container fade-in delay
    ...
    useEffect(() => {
      ...
      const R = 100;
      // 1.5× smaller than before: pull camera back (R*2.85 → R*4.3)
      ...
      // globe group — starts at scale 0, animates up on appear
      ...
      // radial burst pulse — expands & fades on appear
      ...
      // expose internals for the active-watcher useEffect
      ...
      function animate() {
        ...
        if (aAt > 0) {
          ...
          // globe scale 0 → 1 over 1.5s easeOutCubic
          const p = easeOut3(elapsed / 1.5);
          group.scale.setScalar(p);
          // halo fades in alongside globe
          halo.material.opacity = 0.065 * Math.min(1, elapsed / 1.2);

          // radial burst: expands R*0.8 → R*5, fades out over 1.1s
          if (elapsed < 1.15) {
      ...
      return () => {
        ...
        try { if (renderer.domElement.parentNode === el) el.removeChild(renderer.domElement); } catch (e) {}
      };
    }, []);
    ...
    return e('div', {
      ...
      style: {
        /* position/size now in .s7 .globe-container CSS — overridable on mobile */
        zIndex: 5,
  ...
  /* ---- S7StarFlow: stars swirl into the globe on appear ---- */
    ...
    const ref = useFxCanvas({
      ...
      init: (ctx, w, h) => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          // target: spread across globe disc (uniform distribution)
      ...
      frame: (ctx, w, h, t, st) => {
        ...
        for (const p of st.pts) {
          ...
          // gentle swirl toward each particle's own target on the globe
  ...
  function Scene7({ active, onTap }) {
    return e('section', { className: 'scene s7', 'data-active': active, 'data-screen-label': '07' },
      e(window.StarField, { density: 0.16, radiusScale: 0.9, yMax: 0.45 }),
      e(S7StarFlow, { active }),
      e(EarthGlobe, { active }),
      e('div', { className: 'scene-inner' },
  ...
  Object.assign(window, { Scene6, Scene7 });
})();
