...
(function () {
  ...
  (function avatar() {
    ...
    [triL, triM, triS].forEach(t => {
      if (t) t.addEventListener('click', (e) => { e.stopPropagation(); setLevel(+t.dataset.level); });
    });
    const frame = document.getElementById('avatarBlock');
    if (frame) frame.addEventListener('click', () => setLevel(cur === 3 ? 1 : cur + 1));
    setLevel(1);
  ...
  (function paths() {
    ...
    function run(type) {
      ...
      if (type === 'evo') {
        /* Путь эволюции — змейка через все 9 узлов */
        $$('.pd-rows span', block).forEach(s => s.classList.add('on'));
        $$('.pd-cols span', block).forEach(s => s.classList.add('hide'));
        /* Змейка: строка 0 →, вниз, строка 1 ←, вниз, строка 2 → */
        const seq = [
          [0,0],[0,1],[0,2],
          [1,2],[1,1],[1,0],
          [2,0],[2,1],[2,2]
        ...
        function step() {
          ...
          const [r, c] = seq[i];
          at(r, c).classList.add('lit');
          if (i > 0) line(at(seq[i-1][0], seq[i-1][1]), at(r, c));
          i++;
        ...
        /* Путь мастерства — три отдельные вертикали (не соединённые) */
        /* Показываем только столбцы (Молодость/Богатство/Любовь), скрываем строки */
        $$('.pd-cols span', block).forEach(s => s.classList.add('on'));
        $$('.pd-rows span', block).forEach(s => s.classList.add('hide'));
        let seq = [];
        for (let c = 0; c < 3; c++) for (let r = 0; r < 3; r++) seq.push([r, c]);
        let i = 0;
        function step() {
          if (i >= seq.length) return;
          const [r, c] = seq[i];
          at(r, c).classList.add('lit');
          if (r > 0) line(at(r - 1, c), at(r, c)); /* только вертикальные */
          if (r === 2) arrowDown(at(r, c)); /* стрелка вниз в конце каждой колонки */
          i++;
          timer = setTimeout(step, 190);
        }
        step();
        ...
        /* Персональный путь — только 6 узлов, оси скрыты */
        $$('.pd-cols span, .pd-rows span', block).forEach(s => s.classList.add('hide'));
        const seq = [[0,0],[0,2],[1,0],[1,1],[2,0],[2,2]];
        let i = 0;
        function step() {
          if (i >= seq.length) return;
          const [r, c] = seq[i];
    ...
    $$('.pathbtn', block).forEach(b => b.addEventListener('click', () => {
