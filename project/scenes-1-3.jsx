/* scenes-1-3.jsx — Старт / Генезис / Триединая система
   Exports: Scene1, Scene2, Scene3 */
(function () {
  ...
  function Scene1({ active }) {
    return e('section', { className: 'scene s1', 'data-active': active, 'data-screen-label': '01' },
      e(window.StarField, { density: 0.7, twinkle: true }),
      e(window.DustField, { density: 1.4, style: { zIndex: 4 } }),
  ...
  function GenesisFX({ active, sub }) {
    ...
    const ref = window.useFxCanvas({
      ...
      frame: (ctx, w, h, t, st) => {
        ...
        // nodes only — no constellation lines, just scattered universe stars
  ...
  function TriuneFX({ active, sub }) {
    ...
    const ref = window.useFxCanvas({
      ...
      init: (ctx, w, h) => {
        ...
        for (let i = 0; i < 65; i++) {
          ...
          stars.push({
            ...
            ph: Math.random() * Math.PI * 2,
            sp: U.rand(0.4, 1.2),
            // orbit params for the Play-icon halo
            oAng: Math.random() * Math.PI * 2,
            oRadF: U.rand(1.02, 1.42),
            oSpin: U.rand(0.7, 1.25) * (Math.random() < 0.5 ? 1 : 1),
          });
        }
        // contractProg: 0 = stars fully expanded (galaxy size), 1 = contracted to inner circle
      ...
      frame: (ctx, w, h, t, st) => {
        ...
        // ====== LAUNCH: triangle rotates CW → Play icon, stars form orbit ring ======
        if (launching && launchRef.current.t0 > 0) {
          const elapsed = performance.now() / 1000 - launchRef.current.t0;
          const rotE = U.easeInOutCubic(U.clamp(elapsed / 1.35, 0, 1));      // rotation 0→1
          const orbitE = U.easeOutCubic(U.clamp((elapsed - 0.18) / 1.35, 0, 1)); // stars fly out
          const rot = rotE * (Math.PI / 2);     // rotate 90° clockwise → vertex points right
          // breathing: gentle scale pulse once the flip settles (rotE→1), inviting a tap
          const breathe = 1 + rotE * 0.07 * Math.sin(t * 1.8);
          const shrink = (1 - rotE * 0.46) * breathe;   // ~0.54 final (25% smaller), softly breathing
          const detailFade = U.clamp(1 - rotE * 2.4, 0, 1);  // spokes/dots fade out early
          const fillIn = U.clamp((rotE - 0.35) / 0.65, 0, 1); // gold fill fades in
          ...
          // --- stars: fly from inner cluster out to the orbit ring (this IS the circle) ---
          ctx.save();
          for (const s of st.stars) {
            const ix = s.tx * bCos - s.ty * bSin;   // inner (start) position
            const iy = s.tx * bSin + s.ty * bCos;
            const oa = s.oAng + orbitSpin * s.oSpin;
            const oRad = R * 0.92 * s.oRadF;
            const ox = Math.cos(oa) * oRad;         // orbit (end) position
          ...
          // --- the SAME triangle, rotating clockwise + shrinking into a Play glyph ---
          ctx.save();
          ctx.translate(cx, cy);
          ctx.rotate(rot);
          ...
          // gold fill grows in (counter-rotate gradient so it stays left→right shaded)
          if (fillIn > 0.01) {
            ctx.save();
          ...
          // outline (always) — thicker, more golden
          ctx.lineWidth = 3.2;
          ctx.lineJoin = 'round';
          ...
          // inner detail (spokes + dots) only while turning, fading out — no leftover lines
          if (detailFade > 0.02) {
        ...
        // --- contraction: stars shrink from galaxy-scale (4.5x) down to inner-circle (1x)
        ...
        const starScale = 1 + (1 - contractE) * 4.2; // 5.2x → 1x

        // --- triangle appears once stars are ~55% contracted
        const targetA = U.clamp((contractE - 0.5) / 0.5, 0, 1);
        st.appear = (st.appear || 0);
        st.appear += (targetA - st.appear) * 0.055;
        const a = st.appear;

        // --- rotating star cluster (visible throughout, scaled during contraction)
        if (st.stars) {
          st.starRot = (st.starRot || 0) + 0.0009;
          const sCos = Math.cos(st.starRot), sSin = Math.sin(st.starRot);
          const starAlpha = U.clamp(contractE * 3, 0, 1); // fade in at first
          ctx.save();
          if (contractE > 0.97) {
            // fully contracted — clip to inner circle
        ...
        // --- triangle blueprint (fades in as stars contract)
        if (a > 0.02) {
